...
//        other_btn.setText("任务列表");
                ...
                case 1:
                    if (commonBean.getState().equals("success")) {
                        if (commonBean.getData() != null && commonBean.getData().size() != 0) {
                            product_elv.setAdapter(new MyExpandableListViewAdapter(EmployeeActivity.this, commonBean.getData()));
                            product_elv.setGroupIndicator(null);

                            if (product_elv.getCount() > 0) {
                                product_elv.expandGroup(0);
                            }
    ...
    public void showDialogList(Context context, OrderSalesBean bean) {
        ...
        lv.setAdapter(new DialogItemAdapter(EmployeeActivity.this, list));
